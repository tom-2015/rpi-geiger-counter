...
void Socket::SetNonBlocking(){
	if (fcntl(sockDesc, F_SETFL, O_NDELAY) < 0) {
		throw SocketException("Can't set socket to non-blocking", true);
	}
}
...
TCPSocket *TCPServerSocket::accept() throw(SocketException) {
  ...
  if ((newConnSD = ::accept(sockDesc, NULL, 0)) < 0) {
	if (errno ==EAGAIN || errno == EWOULDBLOCK) return NULL; //non blocking mode
    else throw SocketException("Accept failed (accept())", true);
  }
